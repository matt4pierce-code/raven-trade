...
const WS_PAIRS=["btcusdt","ethusdt","solusdt","bnbusdt","xrpusdt","dogeusdt","adausdt","avaxusdt","linkusdt","ltcusdt"];
...
export default function App(){
  ...
  useEffect(()=>{loadSaved().then(s=>{if(s){setHistory(s.history||[]);setCapital(s.capital||STARTING_CAP);setDayPnL(s.dayPnL||0);setDayLog(s.dayLog||[]);setStreak(s.streak||0);}setReady(true);});},[]);
  useEffect(()=>{if(!ready)return;setSaving(true);const t=setTimeout(()=>savePersist({history,capital,dayPnL,dayLog,streak}).then(()=>setSaving(false)),800);return()=>clearTimeout(t);},[history,capital,dayPnL,dayLog,streak,ready]);
    ...
    const loadKlines=async()=>{
      ...
      for(const pair of PAIRS){
        try{
          ...
          const res=await fetch(`https://data-api.binance.vision/api/v3/klines?symbol=${sym}&interval=5m&limit=50`);
          if(res.ok){const data=await res.json();cache[pair]=data.map(k=>parseFloat(k[4]));}
        }catch(e){cache[pair]=[];}
      }
      klinesRef.current=cache;
      setKlines({...cache});
    };
    loadKlines();
  },[ready]);
  ...
  useEffect(()=>{
    if(!ready) return;
    const streams=WS_PAIRS.map(p=>`${p}@miniTicker`).join("/");
    const url=`wss://stream.binance.com:9443/stream?streams=${streams}`;
    const connect=()=>{
      const ws=new WebSocket(url);
      wsRef.current=ws;
      ws.onopen=()=>{setWsStatus("live");setLastUpdate(new Date());};
      ws.onmessage=(e)=>{
        try{
          const msg=JSON.parse(e.data);const d=msg.data;if(!d) return;
          const sym=d.s;const pair=sym.slice(0,-4)+"/USDT";
          if(!PAIRS.includes(pair)) return;
          const ticker={price:parseFloat(d.c),high24:parseFloat(d.h),low24:parseFloat(d.l),change24:((parseFloat(d.c)-parseFloat(d.o))/parseFloat(d.o)*100)};
          tickersRef.current[pair]=ticker;
          setTickers(prev=>({...prev,[pair]:ticker}));
          setLastUpdate(new Date());
          if(klinesRef.current[pair]){const updated=[...klinesRef.current[pair].slice(1),ticker.price];klinesRef.current[pair]=updated;}
          setTrade(t=>{
            if(!t||t.status!=="OPEN"||t.pair!==pair) return t;
  ...
  useEffect(()=>{
    ...
    const scan=()=>{
      const found=[];
      for(const pair of PAIRS){const k=klinesRef.current[pair];const t=tickersRef.current[pair];const sig=analyzeSignal(pair,k,t);if(sig) found.push({...sig,id:`${pair}-${Date.now()}`});}
      if(found.length){setSignals(prev=>{const ex=new Set(prev.map(s=>s.pair));const fresh=found.filter(s=>!ex.has(s.pair));if(fresh.length) notify(`${fresh.length} LIVE signal${fresh.length>1?"s":""} — real Binance data`,"signal");return[...fresh,...prev].slice(0,6);});}
    };
    scanTimer.current=setInterval(scan,30000);
    const init=setTimeout(scan,5000);
    return()=>{clearInterval(scanTimer.current);clearTimeout(init);};
  },[ready,notify]);

  const dailyTarget=capital*DAILY_TARGET;const targetHit=dayPnL>=dailyTarget;const lossHit=dayPnL<=-(capital*DAILY_LOSS_MAX);const streakStop=streak>=CONSEC_LOSS_MAX;const capHit=dayLog.length>=MAX_TRADES_DAY;const locked=targetHit||lossHit||streakStop||capHit;const progress=Math.min(100,Math.max(0,dayPnL/dailyTarget*100));const totalPnL=capital-STARTING_CAP;const wins=history.filter(t=>t.won).length;const losses=history.filter(t=>!t.won).length;const wr=history.length?Math.round(wins/history.length*100):0;const avgGain=history.length>3?history.reduce((a,t)=>a+t.gain,0)/history.length:null;const isOpen=trade&&trade.status==="OPEN";const needsExit=trade&&(trade.status==="TP"||trade.status==="SL");
  const statusColor=wsStatus==="live"?CLR.green:wsStatus==="error"?"#ff4d4d":CLR.yel;
  const statusText=wsStatus==="live"?"● LIVE BINANCE":wsStatus==="error"?"● DISCONNECTED":"● CONNECTING…";

  function enter(sig){if(locked){notify("Trading locked for today.","error");return;}if(isOpen){notify("Close current trade first.","error");return;}const size=capital*RISK_PER_TRADE/HARD_STOP;setTrade({id:sig.id,pair:sig.pair,entry:sig.price,cur:sig.price,size,dir:sig.dir,target:sig.targetPrice,sl:sig.dir==="BUY"?sig.price*(1-HARD_STOP):sig.price*(1+HARD_STOP),why:sig.why,quality:sig.quality,potentialGain:sig.potentialGain,pl:0,status:"OPEN",opened:new Date().toISOString()});setSignals(s=>s.filter(x=>x.id!==sig.id));notify(`Entered ${sig.pair} @ ${usd(sig.price)}`,"success");}
  function closePos(t,approve){if(!approve){setTrade(x=>x?{...x,status:"OPEN"}:x);notify(`Holding ${t.pair}.`,"info");return;}const gain=t.size*t.pl;const won=t.pl>=0;const closed={...t,closed:new Date().toISOString(),gain,won};setCapital(c=>c+gain);setDayPnL(d=>d+gain);setDayLog(d=>[...d,closed]);setHistory(h=>[closed,...h]);setTrade(null);if(won){setStreak(0);}else{setStreak(s=>{const n=s+1;if(n>=CONSEC_LOSS_MAX)notify("2 losses in a row — done for today.","error");return n;});}notify(`${t.pair} ${won?"WIN":"LOSS"} ${usd(Math.abs(gain))}`,won?"success":"error");}
  ...
  return(<div style={{background:CLR.bg,minHeight:"100vh",fontFamily:"'Courier New',monospace",color:"#bbb",paddingBottom:56}}><style>{CSS}</style>{toast&&(<div className="toast" style={{borderColor:toast.type==="success"?CLR.green:toast.type==="error"?CLR.red:toast.type==="signal"?"#9b6dff":"#444",background:toast.type==="success"?"#00ff9d12":toast.type==="error"?"#ff4d4d12":"#7b5ea715"}}>{toast.msg}</div>)}
  <div style={{background:CLR.dark,borderBottom:`1px solid ${CLR.bdr}`,padding:"11px 14px",display:"flex",flexWrap:"wrap",gap:10,alignItems:"center",justifyContent:"space-between"}}><div style={{display:"flex",alignItems:"center",gap:10}}><span style={{color:CLR.green,fontSize:20}}>▲</span><div><div style={{color:"#fff",fontWeight:700,fontSize:14,letterSpacing:3}}>RAVEN TRADE</div><div style={{fontSize:9,letterSpacing:2,display:"flex",gap:6,alignItems:"center"}}><span style={{color:statusColor}}>{statusText}</span>{lastUpdate&&<span style={{color:"#222"}}>{lastUpdate.toLocaleTimeString()}</span>}{saving&&<span style={{color:"#ffd16655"}}>SAVING…</span>}</div></div></div><div style={{display:"flex",gap:14,flexWrap:"wrap"}}>{[["Capital",usd(capital),capital>=STARTING_CAP?CLR.green:CLR.red],["P&L",usd(totalPnL),totalPnL>=0?CLR.green:CLR.red],["Today",usd(dayPnL),dayPnL>=0?CLR.green:CLR.red],["Win%",wr+"%",wr>=60?CLR.green:wr>=45?CLR.yel:CLR.red]].map(([l,v,c])=>(<div key={l} style={{textAlign:"center"}}><div style={{color:"#2a2a2a",fontSize:9,letterSpacing:1}}>{l}</div><div style={{color:c,fontWeight:700,fontSize:12,fontFamily:"monospace"}}>{v}</div></div>))}</div><div style={{display:"flex",gap:6}}><TinyBtn onClick={()=>setPaused(p=>!p)} col={paused?CLR.green:CLR.red}>{paused?"▶":"⏸"}</TinyBtn><TinyBtn onClick={newDay} col="#444">New Day</TinyBtn><TinyBtn onClick={resetAll} col="#ff4d4d44">Reset</TinyBtn></div></div>
  <div style={{background:CLR.dark,borderBottom:`1px solid ${CLR.bdr}`,padding:"10px 14px"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><div style={{display:"flex",gap:8,alignItems:"center"}}><span style={{color:targetHit?CLR.green:CLR.yel,fontWeight:700,fontSize:11,letterSpacing:1}}>{targetHit?"✅ TARGET HIT":"🎯 DAILY MISSION"}</span><span style={{color:CLR.dim,fontSize:10}}>{usd(Math.max(0,dayPnL))} / {usd(dailyTarget)}</span></div><div style={{display:"flex",gap:8,alignItems:"center"}}><div style={{display:"flex",gap:3}}>{[0,1,2].map(i=>(<div key={i} style={{width:10,height:10,borderRadius:"50%",background:dayLog.length>i?CLR.green:CLR.bdr,border:`1px solid ${dayLog.length>i?CLR.green:"#252525"}`,transition:"all 0.3s"}}/>))}<span style={{color:"#222",fontSize:9,marginLeft:3}}>{dayLog.length}/{MAX_TRADES_DAY}</span></div>{streak>0&&<span style={{color:CLR.red,fontSize:9,border:`1px solid ${CLR.red}44`,padding:"1px 6px",borderRadius:3}}>{streak} loss{streak>1?"es":""}</span>}</div></div><div style={{background:"#0d1117",borderRadius:4,height:6,overflow:"hidden"}}><div style={{width:progress+"%",height:"100%",background:targetHit?CLR.green:dayPnL<0?CLR.red:CLR.yel,borderRadius:4,transition:"width 0.8s"}}/></div><div style={{marginTop:5,fontSize:10,color:CLR.dim}}>{targetHit?"Done. Protect your capital.":lossHit?"Daily loss limit hit.":streakStop?"Two losses — walk away.":capHit?"3 trades taken. Done.":`${usd(Math.max(0,dailyTarget-dayPnL))} to target`}</div></div>
  ...
  {tab==="trade"&&<><div style={{background:CLR.dark,border:`1px solid ${CLR.bdr}`,borderRadius:8,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{color:CLR.yel,fontSize:10,fontWeight:700,letterSpacing:2}}>LIVE BINANCE WEBSOCKET</div><div style={{color:"#444",fontSize:10,marginTop:2}}>Real-time prices · BB({BB_PERIOD},{BB_STD_DEV}σ) + RSI + MACD</div></div><div style={{color:statusColor,fontSize:11,fontWeight:700}}>{wsStatus==="live"?"LIVE":"CONNECTING"}</div></div>
  ...
  {isOpen&&(<div style={{background:CLR.dark,border:`1px solid ${trade.pl>=0?CLR.green:CLR.red}33`,borderRadius:10,padding:"12px 14px"}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{color:"#fff",fontWeight:700,fontSize:15}}>{trade.pair} {trade.dir}</span><span style={{color:trade.pl>=0?CLR.green:CLR.red,fontWeight:700,fontSize:15}}>{pct(trade.pl*100)}</span></div><div style={{display:"flex",gap:14,marginTop:8,flexWrap:"wrap"}}>{[["Entry",usd(trade.entry),"#555"],["Now",usd(trade.cur),"#fff"],["P&L",usd(trade.size*trade.pl),trade.pl>=0?CLR.green:CLR.red],["Target",usd(trade.target),CLR.green],["Stop",usd(trade.sl),CLR.red]].map(([l,v,c])=>(<div key={l}><div style={{color:"#2a2a2a",fontSize:9}}>{l}</div><div style={{color:c,fontWeight:700,fontSize:12}}>{v}</div></div>))}</div><button style={{marginTop:8,width:"100%",padding:"8px",background:"transparent",border:"1px solid #ffffff09",color:"#333",borderRadius:6,cursor:"pointer",fontFamily:"inherit",fontSize:10}} onClick={()=>closePos({...trade},true)}>Exit Manually</button></div>)}
  {!locked&&!isOpen&&signals.length===0&&(<div style={{textAlign:"center",padding:"50px 20px"}}><div className="pulse" style={{fontSize:34}}>📡</div><div style={{color:"#1e1e1e",fontSize:12,marginTop:12,letterSpacing:2}}>SCANNING LIVE BINANCE MARKET…</div><div style={{color:"#161616",fontSize:10,marginTop:6}}>Real-time WebSocket · Scans every 30 seconds</div></div>)}
