...
async function loadAllData(klinesCache){
  ...
  try{const data=await fetchWithFallback("https://data-api.binance.vision/api/v3/ticker/24hr");if(data&&Array.isArray(data)){data.forEach(t=>{const formatted=t.symbol.slice(0,-4)+"/USDT";if(PAIRS.includes(formatted)){tickers[formatted]={price:parseFloat(t.lastPrice),high24:parseFloat(t.highPrice),low24:parseFloat(t.lowPrice),change24:parseFloat(t.priceChangePercent)};}});}}catch(e){}
  for(const pair of PAIRS){if(!klines[pair]||klines[pair].length<BB_PERIOD+2){try{const sym=pair.replace("/","");const data=await fetchWithFallback(`https://data-api.binance.vision/api/v3/klines?symbol=${sym}&interval=5m&limit=50`);if(data&&Array.isArray(data)){klines[pair]=data.map(k=>parseFloat(k[4]));}}catch(e){}}else if(tickers[pair]){klines[pair]=[...klines[pair].slice(1),tickers[pair].price];}}
  return{tickers,klines};
}

export default function App(){
  const [ready,setReady]=useState(false);const [tickers,setTickers]=useState({});const [klinesCache,setKlinesCache]=useState({});const [signals,setSignals]=useState([]);const [trade,setTrade]=useState(null);const [history,setHistory]=useState([]);const [dayLog,setDayLog]=useState([]);const [capital,setCapital]=useState(STARTING_CAP);const [dayPnL,setDayPnL]=useState(0);const [streak,setStreak]=useState(0);const [paused,setPaused]=useState(false);const [tab,setTab]=useState("trade");const [toast,setToast]=useState(null);const [saving,setSaving]=useState(false);const [apiStatus,setApiStatus]=useState("connecting");const [lastUpdate,setLastUpdate]=useState(null);const [notifStatus,setNotifStatus]=useState("unknown");
  const klinesCacheRef=useRef({});const isFetching=useRef(false);
  ...
  const pushNotify=useCallback((title,body)=>{
    if("Notification" in window&&Notification.permission==="granted"){
      try{new Notification(title,{body,icon:"https://raven-trade-puce.vercel.app/favicon.ico",vibrate:[200,100,200]});}catch(e){}
    }
  },[]);

  const requestNotifPermission=useCallback(()=>{
    if("Notification" in window){
      Notification.requestPermission().then(p=>{setNotifStatus(p);if(p==="granted") notify("Notifications enabled! You will be alerted when signals fire.","success");});
    }
  },[notify]);
  ...
  const fetchAndUpdate=useCallback(async()=>{
    ...
    try{
      ...
      if(Object.keys(newTickers).length>0){
        setApiStatus("live");setLastUpdate(new Date());setTickers(newTickers);klinesCacheRef.current=newKlines;setKlinesCache({...newKlines});
        setTrade(t=>{if(!t||t.status!=="OPEN") return t;const cur=newTickers[t.pair]?.price;if(!cur) return t;const pl=(cur-t.entry)/t.entry*(t.dir==="BUY"?1:-1);const hitTarget=t.dir==="BUY"?cur>=t.target:cur<=t.target;if(hitTarget) return{...t,status:"TP",cur,pl};if(pl<=-HARD_STOP) return{...t,status:"SL",cur,pl};return{...t,cur,pl};});
        const found=[];for(const pair of PAIRS){const sig=analyzeSignal(pair,newKlines[pair],newTickers[pair]);if(sig) found.push({...sig,id:`${pair}-${Date.now()}`});}
        if(found.length){
          setSignals(prev=>{
    ...
    }catch(e){setApiStatus("error");}finally{isFetching.current=false;}
  },[paused,notify,pushNotify]);

  useEffect(()=>{if(!ready) return;fetchAndUpdate();const id=setInterval(fetchAndUpdate,POLL_MS);return()=>clearInterval(id);},[ready,fetchAndUpdate]);
  ...
  function resetAll(){if(!window.confirm("Reset to $1,000?"))return;setCapital(STARTING_CAP);setDayPnL(0);setDayLog([]);setHistory([]);setTrade(null);setSignals([]);setStreak(0);notify("Reset.","info");}
  ...
  {tab==="trade"&&<>
    <div style={{background:CLR.dark,border:`1px solid ${CLR.bdr}`,borderRadius:8,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{color:CLR.yel,fontSize:10,fontWeight:700,letterSpacing:2}}>LIVE MARKET DATA</div><div style={{color:"#444",fontSize:10,marginTop:2}}>Real Binance · Updates every 15s · BB+RSI+MACD · Mean Reversion</div></div><div style={{color:statusColor,fontSize:11,fontWeight:700}}>{apiStatus==="live"?"LIVE":apiStatus==="error"?"RETRYING":"CONNECTING"}</div></div>
